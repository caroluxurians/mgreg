...
const ArticlePreview = ({ article }: { article: ArticleType }) => {
  ...
  return (
    <div className="text-medium-pink grid grid-cols-2 gap-3 border-b border-light-pink pb-6 sm:gap-4 sm:pb-6 md:flex md:justify-between lg:w-200">
      ...
      <div className="place-self-center md:place-self-start">
        <Image
          ...
          unoptimized
          className="rounded-[40] sm:rounded-[56] md:rounded-[50] lg:rounded-[58] w-40 h-30 object-cover object-top sm:w-60 sm:h-40 md:w-44 md:h-30 lg:w-56 lg:h-38"
        />
      </div>
      ...
      <div className="hidden md:flex flex-col w-70 gap-2 lg:w-74">
        <div className="hidden md:flex font-black text-[16px]/[22px] lg:w-70 lg:text-[18px]/[24px]">
          {article.title}
        </div>
        <div className="hidden md:flex lg:hidden text-[15px]/[22px]">
          {preview}
        </div>
        <div className="hidden lg:flex text-[16px] w-74">{previewL}</div>
      </div>
      <Link
        href={`/clanky${article.link}`}
        className="bg-medium-pink rounded-full text-[14px] text-white font-semibold italic text-center w-fit h-fit place-self-end px-4 py-1 mr-6 sm:text-[16px] md:self-start lg:mt-1 3xl:px-7 3xl:py-2.5"
      >
        číst
      </Link>
    </div>
